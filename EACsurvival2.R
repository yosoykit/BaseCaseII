EACsurvival = function(age = 60, sex = 1, yrdx = 2000, stage = "A", range = 1:100) {
  ...
  # compute survival curve + cure-rate
  surv = Cure[age.ind]+(1-Cure[age.ind])*(1 - plnorm(range, meanlog = mu[age.ind], sdlog = sigma[age.ind]))

  return(list(Cure = Cure[age.ind], sd.Cure = sd.Cure[age.ind], msurv = msurv[age.ind], sd.msurv = sd.msurv[age.ind], surv = surv))
  }
