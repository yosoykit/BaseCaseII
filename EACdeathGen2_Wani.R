EACdeathGen_Wani = function(n=1, age = 60, sex = 1, yrdx = 2000, stage = "U", screen = FALSE) {
  ...
  if(screen==TRUE) {
    lag=18
    surv60mo = (1 - plnorm(60-lag, meanlog = mu, sdlog = sigma)) ## adjust cure prob for screen detected cancers
    Cure = (.68-surv60mo)/(1-surv60mo)
  }  
}
  ...
  if(toupper(stage)== "L" & screen == TRUE) time.candth = time.candth + lag
